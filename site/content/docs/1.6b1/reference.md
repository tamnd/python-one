...
The operators and test for sequence membership: if *y* is a sequence, *`x`*` in `*`y`* is true if and only if there exists an index *i* such that *`x`*` = `*`y`*`[`*`i`*`]`. *`x`*` not in `*`y`* yields the inverse truth value. The exception `TypeError` is raised when *y* is not a sequence, or when *y* is a string and *x* is not a string of length one.[^5]

...
| same as *`s`*`[`*`i`*`:`*`i`*`] = [`*`x`*`]` if *`i`*` >= 0` *s*.remove(*x*) |  |  |
...
Read one entire line from the file. A trailing newline character is kept in the string [^5] (but may be absent when a file ends with an incomplete line). If thevarsize argument is present and non-negative, it is a maximum byte count (including the trailing newline) and an incomplete line may be returned. An empty string is returned when EOF is hit immediately. Note: unlike `stdio`’s `fgets()`, the returned string contains null characters (`’’`) if they occurred in the input.

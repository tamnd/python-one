...
The built-in function `repr()` performs exactly the same conversion in its argument as enclosing it it reverse quotes does. The built-in function `str()` performs a similar but more user-friendly conversion.
...
`continue` may only occur syntactically nested in a `for` or `while` loop, but not nested in a function or class definition or `try` statement within that loop.[^7]

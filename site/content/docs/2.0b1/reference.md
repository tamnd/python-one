...
For the Unicode and string types, *`x`*` in `*`y`* is true if and only if there exists such an index *i* such that *`x`*` == `*`y`*`[`*`i`*`]` is true. If *`x`* is not a string of length `1` or a unicode object of length `1`, a `TypeError` exception is raised.

...
exec-prefix/lib/python1.*X*/site-packages prefix/bin prefix/share
...
The specified installation directories are relative to *prefix*. Of course, you also have to ensure that these directories are in Python’s module search path, e.g. by putting a `.pth` file in *prefix* ().

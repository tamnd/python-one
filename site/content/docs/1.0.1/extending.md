...
There is a straightforward translation from the arguments to the call in Python (here the single expression `’ls -l’`) to the arguments that are passed to the C function. The C function always has two parameters, conventionally named *self* and *args*. The *self* argument is used when the C function implements a builtin method — this is advanced material and not covered in this document. In the example, *self* will always be a `NULL` pointer, since we are defining a function, not a method (this is done so that the interpreter doesn’t have to understand two different types of C functions).

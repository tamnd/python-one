...
atols Convert string *s* to a long integer in the given *base*. The string must consist of one or more digits, optionally preceded by a sign (`+` or `-`). The *base* argument has the same meaning as for `atoi()`. A trailing `l` or `L` is not allowed, except if the base is 0.
...
splitfieldss This function behaves identical to `split`. (In the past, `split` was only used with one argument, while `splitfields` was only used with two arguments.)
...
removepath Remove the file *path*. See `rmdir` below to remove a directory.
...
escape: convert the characters “`&`”, “`<`” and “`>`” to HTML-safe sequences. Use this if you need to display text that might contain such characters in HTML. To translate URLs for inclusion in the HREF attribute of an `<A>` tag, use `urllib.quote()`.
